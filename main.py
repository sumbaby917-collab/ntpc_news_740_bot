...
if api_key:
    ...
    # 修正重點：確保模型名稱為 'gemini-1.5-flash'，後方無雜質
...
def get_ai_analysis(title):
    if not model: return "摘要：API未設定。\n因應：請檢查系統環境。"
    prompt = f"你是一位新北市教育局官員。針對新聞「{title}」，請直接產出兩行文字：『摘要：(兩句話)』與『因應：(具體作為)』。"
    try:
        response = model.generate_content(prompt)
        return response.text.strip() if response.text else "無法產生內容"
    except Exception as e:
        # 這裡會捕捉錯誤，如果是 404 代表模型名稱還是有問題
        return f"摘要：分析失敗。\n因應：持續監控輿情。({str(e)[:30]})"
